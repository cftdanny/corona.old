...
public class CoronaApplicationModule extends WebStartModule {
	...
	private void configureStaticContent() {
		...
		this.bindConfiguration(Handler.class).as("script").property("pattern").value("/script");
		...
		this.bindConfiguration(Handler.class).as("style").property("pattern").value("/style");
		...
		this.bindConfiguration(Handler.class).as("image").property("pattern").value("/image");
