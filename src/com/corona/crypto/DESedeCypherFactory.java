...
/**
 * <p>the cypher factory for DESede </p>
 *
 * @author $Author$
 * @version $Id$
 */
public class DESedeCypherFactory extends CypherFactory {

	/**
	 * the factory name
	 */
	public static final String NAME = "DESede";
	...
	public CertifiedKey generateKey(final int size) throws CypherException {
		...
		try {
			generator = KeyGenerator.getInstance(DESedeCypher.ALGORITHM);
		} catch (Exception e) {
			throw new CypherException(
					"Fail to get key generator for [{0}] cypher", e, DESedeCypher.ALGORITHM
			);
		}
		...
		try {
			...
			throw new CypherException(
					"Fail to initial [{0}] cypher with key size [{1}]", e, DESedeCypher.ALGORITHM, size
			);
		}
