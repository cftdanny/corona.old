...
/**
 * <p>the cypher factory for AES </p>
 *
 * @author $Author$
 * @version $Id$
 */
public class AESCypherFactory extends CypherFactory {
	...
	public CertifiedKey generateKey(final int size) throws CypherException {
		...
		try {
			generator = KeyGenerator.getInstance(AESCypher.ALGORITHM);
		} catch (Exception e) {
			throw new CypherException(
					"Fail to get key generator for [{0}] cypher", e, AESCypher.ALGORITHM
			);
		}
		...
		try {
			...
			throw new CypherException(
					"Fail to initial [{0}] cypher with key size [{1}]", e, AESCypher.ALGORITHM, size
			);
		}
